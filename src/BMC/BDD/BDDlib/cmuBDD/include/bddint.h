...
/* do seem to be broken.  If your C preprocessor is ANSI, this shouldn't */
/* have any effect. */

/* #define BROKEN_CPP */

/* Define this if your C preprocessor is ANSI, but your compiler isn't. */
...
#if defined(__STDC__) || defined(ANSI_CPP)
...
struct cache_
{
  ...
  long (*rehash_fn[8]) ARGS((bdd_manager, cache_entry));
				/* Rehashes a cache entry */
  int (*gc_fn[8]) ARGS((bdd_manager, cache_entry));
				/* Checks to see if an entry needs flushing */
  void (*purge_fn[8]) ARGS((bdd_manager, cache_entry));
				/* Called when purging an entry */
  void (*return_fn[8]) ARGS((bdd_manager, cache_entry));
				/* Called before returning from cache hit */
  int (*flush_fn[8]) ARGS((bdd_manager, cache_entry, pointer));
...
struct unique_
{
  ...
  void (*free_terminal_fn) ARGS((bdd_manager, INT_PTR, INT_PTR, pointer));
...
struct bdd_manager_
{
  ...
  void (*bag_it_fn) ARGS((bdd_manager, pointer));
				/* Non-null if going to abort at next find */
  pointer bag_it_env;		/* Environment for bag it function */
};
...
extern int bdd_type_aux ARGS((bdd_manager, bdd));
extern void bdd_rehash_var_table ARGS((var_table, int));
extern bdd bdd_find_aux ARGS((bdd_manager, bdd_indexindex_type, INT_PTR, INT_PTR));
extern bdd bdd_ite_step ARGS((bdd_manager, bdd, bdd, bdd));
extern bdd bdd_exists_temp ARGS((bdd_manager, bdd, long));
extern bdd bdd_compose_temp ARGS((bdd_manager, bdd, bdd, bdd));
extern bdd bdd_substitute_step ARGS((bdd_manager, bdd, long, bdd (*) ARGS((bdd_manager, bdd, bdd, bdd)), var_assoc));
extern bdd bdd_swap_vars_temp ARGS((bdd_manager, bdd, bdd, bdd));
extern int bdd_compare_temp ARGS((bdd_manager, bdd, bdd, bdd));
...
extern void mtbdd_terminal_value_aux ARGS((bdd_manager, bdd, INT_PTR *, INT_PTR *));


/* System cache routines */

extern void bdd_insert_in_cache31 ARGS((bdd_manager, int, INT_PTR, INT_PTR, INT_PTR, INT_PTR));
extern int bdd_lookup_in_cache31 ARGS((bdd_manager, int, INT_PTR, INT_PTR, INT_PTR, INT_PTR *));
extern void bdd_insert_in_cache22 ARGS((bdd_manager, int, INT_PTR, INT_PTR, INT_PTR, INT_PTR));
extern int bdd_lookup_in_cache22 ARGS((bdd_manager, int, INT_PTR, INT_PTR, INT_PTR *, INT_PTR *));
extern void bdd_insert_in_cache13 ARGS((bdd_manager, int, INT_PTR, INT_PTR, INT_PTR, INT_PTR));
extern int bdd_lookup_in_cache13 ARGS((bdd_manager, int, INT_PTR, INT_PTR *, INT_PTR *, INT_PTR *));
extern void bdd_flush_cache ARGS((bdd_manager, int (*) ARGS((bdd_manager, cache_entry, pointer)), pointer));
extern void bdd_purge_cache ARGS((bdd_manager));
extern void bdd_flush_all ARGS((bdd_manager));
extern int bdd_cache_functions ARGS((bdd_manager,
				     int,
				     int (*) ARGS((bdd_manager, cache_entry)),
				     void (*) ARGS((bdd_manager, cache_entry)),
				     void (*) ARGS((bdd_manager,cache_entry)),
				     int (*) ARGS((bdd_manager, cache_entry, pointer))));
extern void bdd_free_cache_tag ARGS((bdd_manager, int));
extern void bdd_rehash_cache ARGS((bdd_manager, int));
...
#define cache_purge_fn_none ((void (*)(bdd_manager, cache_entry))0)
#define cache_reclaim_fn_none ((int (*)(bdd_manager, cache_entry, pointer))0)
#else
...
extern void bdd_clear_temps ARGS((bdd_manager));
extern void bdd_sweep_var_table ARGS((bdd_manager, long, int));
extern void bdd_sweep ARGS((bdd_manager));
extern void bdd_cleanup ARGS((bdd_manager, int));
extern bdd bdd_find ARGS((bdd_manager, bdd_indexindex_type, bdd, bdd));
extern bdd bdd_find_terminal ARGS((bdd_manager, INT_PTR, INT_PTR));
extern var_table bdd_new_var_table ARGS((bdd_manager));
extern void bdd_init_unique ARGS((bdd_manager));
extern void bdd_free_unique ARGS((bdd_manager));
...
extern hash_table bdd_new_hash_table ARGS((bdd_manager, int));
extern void bdd_free_hash_table ARGS((hash_table));
