  ...
  if (!mgr->free)
    {
      ...
      _new=(list)mem_get_block(ALLOC_SIZE);
      _new->next=mgr->blocks;
      mgr->blocks=_new;
      mgr->free=(list)((INT_PTR)_new+ROUNDUP(sizeof(struct list_)));
      p=(pointer)(mgr->free);
...
#if defined(DEBUG_MEM)
  if (mgr->size >= sizeof(long)+sizeof(struct list_))
    if (*(long *)(sizeof(struct list_)+(INT_PTR)_new) != MAGIC_COOKIE)
      fprintf(stderr, "record at 0x%lx may be in use\n", (INT_PTR)_new);
    else
      *(long *)(sizeof(struct list_)+(INT_PTR)_new)=MAGIC_COOKIE1;
#endif
