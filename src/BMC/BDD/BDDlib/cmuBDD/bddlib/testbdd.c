...
#define ITERATIONS 2000		/* Number of trials to run */
...
#endif
{
  ...
  if (var == TT_VARS)
    return ((table & 1) ? bdd_one(bddm) : bdd_zero(bddm));
  temp1=decode(var+1, table >> (1 << (TT_VARS-var-1)));
  temp2=decode(var+1, table);
  result=bdd_ite(bddm, vars[var], temp1, temp2);
  bdd_free(bddm, temp1);
  bdd_free(bddm, temp2);
  return (result);
}
...
char *
#if defined(__STDC__)
terminal_id_fn(bdd_manager bddm, INT_PTR v1, INT_PTR v2, pointer junk)
#else
...
#endif
{
  bdd_print_bdd(bddm, f, bdd_naming_fn_none, terminal_id_fn, (pointer)0, stderr);
}
...
#endif
{
  ...
  var=((unsigned long)rand())%TT_VARS;
  result=bdd_compose(bddm, f1, vars[var], bdd_one(bddm));
  ...
  bdd_free(bddm, result);
  bdd_free(bddm, expected);
  result=bdd_compose(bddm, f1, vars[var], bdd_zero(bddm));
  ...
  bdd_free(bddm, result);
  bdd_free(bddm, expected);
  result=bdd_compose(bddm, f1, vars[var], f2);
...
#endif
{
  ...
  var1=((unsigned long)rand())%TT_VARS;
  do
    var2=((unsigned long)rand())%TT_VARS;
  while (var1 == var2);
  assoc[0]=vars[var1];
  assoc[1]=vars[var2];
  assoc[2]=0;
  bdd_temp_assoc(bddm, assoc, 0);
  bdd_assoc(bddm, -1);
  if (rand()%2)
    result=bdd_exists(bddm, f);
  else
    {
      temp=bdd_not(bddm, f);
      result=bdd_forall(bddm, temp);
      bdd_free(bddm, temp);
      temp=result;
      result=bdd_not(bddm, temp);
      bdd_free(bddm, temp);
    }
...
#endif
{
  ...
  bdd_temp_assoc(bddm, assoc, 0);
  bdd_assoc(bddm, -1);
  result=bdd_rel_prod(bddm, f1, f2);
  table1&=table2;
...
#endif
{
  ...
  result=bdd_intersects(bddm, f1, f2);
  resulttable=table1 & table2;
  expected=encoding_to_bdd(resulttable);
  implies_result=bdd_implies(bddm, result, expected);
  if (implies_result != bdd_zero(bddm))
    {
      error("intersection test", result, expected, f1, f2, (bdd)0);
      bdd_free(bddm, implies_result);
    }
  bdd_free(bddm, result);
  bdd_free(bddm, expected);
}
...
#endif
{
  ...
  if (f == bdd_zero(bddm))
    return;
  result=bdd_satisfy(bddm, f);
  temp1=bdd_not(bddm, f);
  temp2=bdd_intersects(bddm, temp1, result);
  if (temp2 != bdd_zero(bddm))
    error("intersection of satisfy result with negated argument", temp2, bdd_zero(bddm), f, (bdd)0);
  bdd_free(bddm, temp1);
  bdd_free(bddm, temp2);
  var1=((unsigned long)rand())%TT_VARS;
  do
  ...
  bdd_temp_assoc(bddm, assoc, 0);
  bdd_assoc(bddm, -1);
  temp1=bdd_satisfy_support(bddm, result);
  temp2=bdd_not(bddm, result);
  temp3=bdd_intersects(bddm, temp2, temp1);
  if (temp3 != bdd_zero(bddm))
    error("intersection of satisfy support result with negated argument", temp3, bdd_zero(bddm), result, (bdd)0);
  bdd_free(bddm, result);
  bdd_free(bddm, temp1);
  bdd_free(bddm, temp2);
  bdd_free(bddm, temp3);
  temp1=bdd_compose(bddm, f, vars[var1], bdd_zero(bddm));
  temp2=bdd_compose(bddm, f, vars[var1], bdd_one(bddm));
  if (bdd_satisfying_fraction(bddm, temp1)+bdd_satisfying_fraction(bddm, temp2) !=
      2.0*bdd_satisfying_fraction(bddm, f))
    {
...
#endif
{
  ...
  result=bdd_cofactor(bddm, f1, f2);
  temp1=bdd_xnor(bddm, result, f1);
  temp2=bdd_not(bddm, f2);
  temp3=bdd_or(bddm, temp1, temp2);
  if (temp3 != bdd_one(bddm))
    error("d.c. comparison of generalized cofactor", temp3, bdd_one(bddm), f1, f2, (bdd)0);
  bdd_free(bddm, result);
  bdd_free(bddm, temp1);
  bdd_free(bddm, temp2);
  bdd_free(bddm, temp3);
  ...
  do
    var2=((unsigned long)rand())%TT_VARS;
  while (var1 == var2);
  temp1=bdd_not(bddm, vars[var2]);
  temp2=bdd_and(bddm, vars[var1], temp1);
  bdd_free(bddm, temp1);
  result=bdd_cofactor(bddm, f1, temp2);
...
#endif
{
  ...
  result=bdd_reduce(bddm, f1, f2);
  temp1=bdd_xnor(bddm, result, f1);
  temp2=bdd_not(bddm, f2);
  temp3=bdd_or(bddm, temp1, temp2);
  if (temp3 != bdd_one(bddm))
    error("d.c. comparison of reduce", temp3, bdd_one(bddm), f1, f2, (bdd)0);
  bdd_free(bddm, result);
  bdd_free(bddm, temp1);
  bdd_free(bddm, temp2);
  bdd_free(bddm, temp3);
...
bdd
#if defined(__STDC__)
apply_and(bdd_manager bddm, bdd *f, bdd *g, pointer env)
#else
...
#endif
{
  ...
  size=bdd_size(bddm, f1, 1);
  bdd_profile(bddm, f1, profile, 1);
  ...
  size=bdd_size(bddm, f1, 0);
  bdd_profile(bddm, f1, profile, 0);
  for (i=0; i < 2*TT_VARS+1; ++i)
    size-=profile[i];
  if (size)
    {
  ...
  fs[1]=f2;
  fs[2]=0;
  size=bdd_size_multiple(bddm, fs, 1);
  bdd_profile_multiple(bddm, fs, profile, 1);
  for (i=0; i < 2*TT_VARS+1; ++i)
    size-=profile[i];
  if (size)
    {
...
int
#if defined(__STDC__)
canonical_fn(bdd_manager bddm, INT_PTR v1, INT_PTR v2, pointer env)
#else
...
void
#if defined(__STDC__)
transform_fn(bdd_manager bddm, INT_PTR v1, INT_PTR v2, INT_PTR *r1, INT_PTR *r2, pointer env)
#else
...
#endif
{
  ...
  as_INT_PTRs(n, &v1, &v2);
  return (mtbdd_get_terminal(bddm, v1, v2));
}
...
#endif
{
  ...
  temp3=temp2;
  temp2=mtbdd_ite(bddm, aux_vars[n], temp3, temp1);
  bdd_free(bddm, temp3);
  result=mtbdd_ite(bddm, vars[n], temp2, temp1);
  bdd_free(bddm, temp1);
  bdd_free(bddm, temp2);
  return (result);
}
...
bdd
#if defined(__STDC__)
mtbdd_mult_step(bdd_manager bddm, bdd f, bdd g)
#else
...
#endif
{
  ...
  if (BDD_IS_CONST(f) && BDD_IS_CONST(g))
    {
      mtbdd_terminal_value_aux(bddm, f, &u1, &u2);
...
bdd
#if defined(__STDC__)
mtbdd_mult(bdd_manager bddm, bdd f, bdd g)
#else
mtbdd_mult(bddm, f, g)
     bdd_manager bddm;
     bdd f;
...
bdd
#if defined(__STDC__)
mtbdd_add_step(bdd_manager bddm, bdd f, bdd g)
#else
...
bdd
#if defined(__STDC__)
mtbdd_add(bdd_manager bddm, bdd f, bdd g)
#else
mtbdd_add(bddm, f, g)
     bdd_manager bddm;
     bdd f;
...
#endif
{
  ...
  for (i=0; i < TT_VARS; ++i)
    {
      temp1=bdd_compose(bddm, result, elim_vars[i], bdd_one(bddm));
      temp2=bdd_compose(bddm, result, elim_vars[i], bdd_zero(bddm));
      bdd_free(bddm, result);
...
#endif
{
  ...
  var1=((unsigned long)rand())%TT_VARS;
  var2=((unsigned long)rand())%TT_VARS;
  result=bdd_swap_vars(bddm, f, vars[var1], vars[var2]);
...
#endif
{
  ...
  for (i=0; i < TT_VARS-1; ++i)
    {
      j=i+((unsigned long)rand())%(TT_VARS-i);
      temp=dump_vars[i];
      dump_vars[i]=dump_vars[j];
      dump_vars[j]=temp;
    }
  if (!bdd_dump_bdd(bddm, f, dump_vars, fp))
    {
  ...
  rewind(fp);
  if (!(result=bdd_undump_bdd(bddm, dump_vars, fp, &err)) || err)
    {
...
#endif
{
  ...
  bdd_temp_assoc(bddm, assoc, 0);
  bdd_gc(bddm);
  if (bdd_total_size(bddm) != 2*TT_VARS+1l)
...
#endif
{
  ...
  bddm=bdd_init();
  bdd_node_limit(bddm, 5000);
  mtbdd_transform_closure(bddm, canonical_fn, transform_fn, (pointer)0);
  as_INT_PTRs(-1.0, &v1, &v2);
  mtbdd_one_data(bddm, v1, v2);
  vars[1]=bdd_new_var_last(bddm);
  vars[0]=bdd_new_var_first(bddm);
  vars[4]=bdd_new_var_after(bddm, vars[1]);
  vars[3]=bdd_new_var_before(bddm, vars[4]);
  vars[2]=bdd_new_var_after(bddm, vars[1]);
  for (i=0; i < 5; ++i)
    aux_vars[i]=bdd_new_var_after(bddm, vars[i]);
...
#endif
{
  ...
  for (i=0; i < 10; ++i)
    {
      printf("Starting random test suite %d: %ld bytes allocated\n", i+1, (long)mem_allocation());
      random_tests(ITERATIONS);
      printf("End of random test suit %d: %ld bytes allocated\n", i+1, (long)mem_allocation());
    }
}
