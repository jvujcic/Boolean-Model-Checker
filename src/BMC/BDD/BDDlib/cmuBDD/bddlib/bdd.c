...
/* bdd_implies(bddm, f, g) is analogous to bdd_intersects, but it */
...
#endif
{
  ...
  if ((alloc=mem_allocation())) 
  {
    ...
    sprintf(fp, "Memory manager bytes allocated: %ld\n", (long)alloc);
	strcpy(stat, fp);
  }

  sprintf(fp, "Approximate bytes used: %ld\n", mem);
  ...
  sprintf(fp, "Number of nodes: %ld\n", ue);
  strcat(stat,fp);

  if (bddm->unique_table.node_limit)
  {
    sprintf(fp, "Node limit: %ld\n", bddm->unique_table.node_limit);
	strcat(stat, fp);
  }
  else
  {
    sprintf(fp, "Node limit: ---\n");
	strcat(stat, fp);
  }
  sprintf(fp, "Overflow: %s\n", bddm->overflow ? "yes" : "no");
  strcat(stat, fp);
  sprintf(fp, "Approximate bytes per node: %-.2f\n", ((double)mem)/ue);
  strcat(stat, fp);
  sprintf(fp, "Cache entries: %ld\n", ce);
  strcat(stat, fp);
  sprintf(fp, "Cache size: %ld\n", 2*cs);
  strcat(stat, fp);
  sprintf(fp, "Cache load factor: %-.2f\n", ((double)ce)/(2*cs));
  strcat(stat, fp);
  sprintf(fp, "Cache look ups: %ld\n", bddm->op_cache.lookups);
  strcat(stat, fp);
  sprintf(fp, "Cache hits: %ld\n", bddm->op_cache.hits);
  strcat(stat, fp);
  if (bddm->op_cache.lookups)
  {
    sprintf(fp, "Cache hit rate: %-.2f\n", ((double)(bddm->op_cache.hits))/bddm->op_cache.lookups);
	strcat(stat, fp);
  }
  else
  {
    sprintf(fp, "Cache hit rate: ---\n");
	strcat(stat, fp);
  }
  sprintf(fp, "Cache insertions: %ld\n", bddm->op_cache.inserts);
  strcat(stat, fp);
  sprintf(fp, "Cache collisions: %ld\n", bddm->op_cache.collisions);
  strcat(stat, fp);
  sprintf(fp, "Number of variables: %ld\n", bddm->vars);
  strcat(stat, fp);
  sprintf(fp, "Number of variable associations: %d\n", i);
  strcat(stat, fp);
  sprintf(fp, "Number of garbage collections: %ld\n", bddm->unique_table.gcs);
  strcat(stat, fp);
  sprintf(fp, "Number of nodes garbage collected: %ld\n", bddm->unique_table.freed);
  strcat(stat, fp);
  sprintf(fp, "Number of find operations: %ld\n", bddm->unique_table.finds);
  strcat(stat, fp);
